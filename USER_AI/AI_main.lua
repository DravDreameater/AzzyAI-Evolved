...
-- Define a global or external flag to track healing completion
HealingComplete = false

function DoHealingTasks(myid)
    -- Initialize variables:
    rhp = HPPercent(myid)  -- Character's current HP percentage.
    ohp = HPPercent(GetV(V_OWNER, myid))  -- Owner's current HP percentage.
    skill, level = GetHealingSkill(myid)  -- Get healing skill and its level.

    -- If no healing skill is available or skill is unlearned or in cooldown, exit function.
    if skill <= 0 then
        UseAutoHeal = 0  -- Disable auto-heal if no skill.
        return
    elseif level == 0 or GetTick() < AutoSkillTimeout then
        return  -- Skill is unlearned or on cooldown.
    end

    -- Healing logic starts here
    if rhp < HealSelfHP then
        -- Attempt self-healing if below threshold
        if skill == HVAN_CHAOTIC and GetV(V_SP, myid) > GetSkillInfo(skill, 3, level) then
            BetterMoveToOwner(myid, 1)
            MyState = STOP_CMD_ST  -- Change state to stop for healing.
            TraceAI("Using self Healing skill "..FormatSkill(skill, level))

            -- Heal owner or self based on HP.
            if ohp < HealOwnerHP then
                DoSkill(skill, 5, GetV(V_OWNER, myid))  -- Heal owner.
            else
                DoSkill(skill, 4, GetV(V_OWNER, myid))  -- Heal self.
            end
            HealingComplete = false  -- Reset flag since healing is ongoing.
            return
        else
            TraceAI("Can't use self healing skill, no SP")
        end
    elseif rhp == 100 and ohp == 100 then
        -- Both character and owner are fully healed, transition to idle only once.
        if not HealingComplete then
            MyState = IDLEWALK_ST
            TraceAI("Healing complete, switching to idle.")
            HealingComplete = true  -- Set the flag to prevent re-entering idle repeatedly.
        end
    end

    -- Check if owner's HP is below the threshold for healing.
    if ohp < HealOwnerHP then
        if GetV(V_SP, myid) > GetSkillInfo(skill, 3, level) then
            BetterMoveToOwner(myid, 1)
            MyState = STOP_CMD_ST  -- Stop current actions to heal.
            TraceAI("Using Healing skill "..FormatSkill(skill, level))
            DoSkill(skill, level, GetV(V_OWNER, myid))  -- Heal owner.
            HealingComplete = false  -- Reset flag since healing is ongoing.
            return
        else
